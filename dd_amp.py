...
def main():
    ...
    # train
    with open('./csv/{}_cifar10_{}epochs_train.csv'.format(args.model_fullname, args.epoch),'w') as file:
        file.write("epoch,error,loss\n")
        file.write(f"0,{1.0 - train_acc},{train_loss}" + "\n")
    # test
    with open('./csv/{}_cifar10_{}epochs_test.csv'.format(args.model_fullname, args.epoch),'w') as file:
        file.write("epoch,error,loss\n")
    ...
    for epoch in range(epoch):
        ...
        for batich_idx, (inputs, targets) in enumerate(train_loader):
            ...
            x1.append(xpoint)
            ## csv writer
            with open('./csv/{}_cifar10_{}epochs_train.csv'.format(args.model_fullname, args.epoch),'a') as file:
        ...
        test_acc, test_loss = test(model, device, test_loader, criterion)
        # csv writer
        with open('./csv/{}_cifar10_{}epochs_test.csv'.format(args.model_fullname, args.epoch),'a') as file:
    ...
    f'./checkpoint/{args.model_fullname}_{(epoch + 1)}epochs_cifar10.tar')
